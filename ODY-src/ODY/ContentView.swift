...
struct ContentView: View {
    ...
    @AppStorage("ODYDiscordLoggedIn") private var discordLoggedIn = false
    ...
    var body: some View {
        ...
        .onReceive(NotificationCenter.default.publisher(for: .discordLoginStateUpdated)) { note in
            if let loggedIn = note.userInfo?["loggedIn"] as? Bool {
                discordLoggedIn = loggedIn
            }
        }
    }
        ...
        ZStack(alignment: .bottom) {
            ...
            HStack(spacing: 12) {
                Button {
                    ...
                    Label(
                        discordLoggedIn ? "Discord Connected" : "Log in to Discord",
                        systemImage: discordLoggedIn ? "checkmark.circle.fill" : "person.crop.circle"
                    )
        ...
        ZStack(alignment: .top) {
            ...
            HStack(spacing: 12) {
                ...
                VStack(spacing: 1) {
                    ...
                    Text(discordLoggedIn ? "Logged in inside ODY" : "Log in inside ODY")
                        .font(.system(size: 10))
...
extension Notification.Name {
    ...
    static let discordLoginStateUpdated = Notification.Name("discordLoginStateUpdated")
}
